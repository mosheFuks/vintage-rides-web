...
**Curación final a 7 modelos puntuales (misma rama):** el cliente pidió una lista final de 11 modelos específicos. 4 no existían tal cual (color inventado o variante inexistente: "Chevrolet 1954 verde", "Kaiser Carabela 1959 blanco", "Ford A 1930 rojo/verde") — se consultó al cliente y confirmó omitirlos en vez de inventar el dato (regla del proyecto). `src/data/vehiculos.ts` quedó con **7 vehículos**: `ford-a-1930-azul`, `mercury-1947-negro` (antiguos), `kaiser-carabela-1959` (clásicos), `limousine-chrysler-pt` (limousinas), `mercedes-benz-clase-e-2013`, `mercedes-benz-e400-2020`, `audi-a4-azul` (lujo) — todos recuperados del historial de git (commit `d28ee5a`, tip de `main`, inventario completo de 206). `categorias.ts`/`CategoriaId` quedaron en 4 categorías: antiguos, clásicos, limousinas, lujo. Único destacado restante: `mercedes-benz-e400-2020`. Detalle en `PENDIENTES-CLIENTE.md`. Verificado con `npx tsc -b`, `npm run lint` y `npm run build` (18 páginas: 7 vehículos + 4 categorías + 7 estáticas).
