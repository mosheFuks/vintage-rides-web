...
**Cambio fuera del plan (rama `moshi-cars`):** a pedido del cliente, el catálogo se redujo de 8 categorías a 3: **Autos antiguos**, **Autos modernos** y **Limousinas y colectivos antiguos** (primer intento fue con Clásicos en vez de Modernos; el cliente pidió el cambio después, ver detalle en `PENDIENTES-CLIENTE.md`). `src/data/vehiculos.ts` quedó con **53 vehículos** (27 antiguos + 15 modernos + 11 limousinas); los modernos se recuperaron del historial de git (commit `d28ee5a`, tip de `main`, que conserva el inventario completo de 206). `CategoriaId` (`src/types/index.ts`) y `src/data/categorias.ts` reflejan las 3 categorías vigentes. Se ajustó la copy de `CategoriasGrid.tsx` y `data/about.ts` que mencionaba categorías eliminadas (el resto de los conteos en Home/Nosotros/Prueba social son dinámicos vía `VEHICULOS.length`/`CATEGORIAS.length`, no hizo falta tocarlos). Detalle y nota de recuperación en `PENDIENTES-CLIENTE.md` → "Reducción de categorías (post Fase 9)". Verificado con `npx tsc -b`, `npm run lint` y `npm run build` (63 páginas: 53 vehículos + 3 categorías + 7 estáticas, antes de todo esto eran 221).
