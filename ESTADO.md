...
**Cambio fuera del plan (rama `moshi-cars`):** a pedido del cliente, el catálogo pasó por varias reducciones sucesivas (8 categorías → 3 con Clásicos → 3 con Modernos en vez de Clásicos, ver commits anteriores en esta rama), hasta la curación final descripta más abajo. `CategoriaId` (`src/types/index.ts`) y `src/data/categorias.ts` van reflejando en cada paso las categorías vigentes. Se ajustó la copy de `CategoriasGrid.tsx`, `data/about.ts` y la descripción SEO de `Catalogo.tsx` en cada paso (el resto de los conteos en Home/Nosotros/Prueba social son dinámicos vía `VEHICULOS.length`/`CATEGORIAS.length`, no hizo falta tocarlos). Detalle completo del historial en `PENDIENTES-CLIENTE.md` → "Reducción de categorías (post Fase 9)".

**Curación final a 7 modelos puntuales (misma rama):** el cliente pidió una lista final de 11 modelos específicos. 4 no existían tal cual (color inventado o variante inexistente: "Chevrolet 1954 verde", "Kaiser Carabela 1959 blanco", "Ford A 1930 rojo/verde") — se consultó al cliente y confirmó omitirlos en vez de inventar el dato (regla del proyecto). `src/data/vehiculos.ts` quedó con **7 vehículos**, todos recuperados del historial de git (commit `d28ee5a`, tip de `main`, inventario completo de 206).

**Recategorización a las 3 categorías acordadas (misma rama):** el cliente recordó que habían quedado solo 3 categorías: **Antiguos**, **Modernos** y **Limousinas** (no 4). De los 7 vehículos, 3 venían de "Lujo" en los datos originales (`mercedes-benz-clase-e-2013`, `mercedes-benz-e400-2020`, `audi-a4-azul`) y 1 de "Clásicos" (`kaiser-carabela-1959`) — categorías que ya no existen en el sitio. Se consultó al cliente: los 3 de Lujo pasan a **Modernos** (son autos actuales) y el Kaiser Carabela 1959 pasa a **Antiguos** (se amplía un poco esa categoría para autos de mitad de siglo). Se actualizó el campo `categoria` de esos 4 vehículos y su `descripcionCorta`/`descripcionLarga` (que mencionaban "auto clásico"/"auto de lujo") para que coincidan con la nueva categoría. Reparto final: **antiguos** (3: `ford-a-1930-azul`, `mercury-1947-negro`, `kaiser-carabela-1959`), **modernos** (3: `mercedes-benz-clase-e-2013`, `mercedes-benz-e400-2020`, `audi-a4-azul`), **limousinas** (1: `limousine-chrysler-pt`). `categorias.ts`/`CategoriaId` quedaron en esas 3. Único destacado: `mercedes-benz-e400-2020` (ahora en "modernos"). Verificado con `npx tsc -b`, `npm run lint` y `npm run build` (17 páginas: 7 vehículos + 3 categorías + 7 estáticas).

**Trabajos vaciado y oculto (misma rama):** a pedido del cliente, `src/data/trabajos.ts` quedó con `TRABAJOS: Trabajo[] = []` (se borraron los 6 registros; el original sigue recuperable desde `d28ee5a` si hace falta). El link "Trabajos" se sacó de la nav de `Header.tsx` y `Footer.tsx`, pero la ruta `/trabajos` sigue existiendo (accesible por URL directa, con estado vacío "No hay trabajos con ese filtro.") para poder cargar contenido ahí más adelante sin tocar código. Se agregó `noindex` a su `Seo` mientras esté vacía. El strip de trabajos de `PruebaSocial` (Home) y el stat "Producciones documentadas" de `Nosotros` ahora se ocultan condicionalmente cuando `TRABAJOS.length === 0` (en `Nosotros` el grid de stats pasa de 3 a 2 columnas para no dejar un hueco). La sección "Trabajos realizados con este vehículo" de `Auto.tsx` ya se ocultaba sola al no haber datos (no necesitó cambios). Verificado con `npx tsc -b`, `npm run lint` y `npm run build`.
