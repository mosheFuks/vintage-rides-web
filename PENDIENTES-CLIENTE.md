...
A pedido del cliente, el catálogo pasó por varias reducciones sucesivas desde las 8 categorías originales (ver historial de commits en la rama `moshi-cars`: primero a 3 categorías con Clásicos, después con Modernos en vez de Clásicos). Después se curó a **11 modelos puntuales pedidos por el cliente**, de los cuales 7 tenían match exacto en los datos originales (4 no existían tal cual se pidieron — color inventado o variante inexistente — y el cliente confirmó omitirlos en vez de inventar el dato, ver detalle abajo). Esos 7 traían consigo 2 categorías de más ("Clásicos" y "Lujo", que ya no existían en el sitio); el cliente confirmó que las 3 categorías acordadas seguían siendo **Antiguos, Modernos y Limousinas**, así que se recategorizaron: el Kaiser Carabela 1959 (era "Clásicos") pasó a **Antiguos**, y los 3 de "Lujo" (Mercedes Clase E 2013, Mercedes E400 2020, Audi A4 Azul) pasaron a **Modernos**. Se ajustó también su `descripcionCorta`/`descripcionLarga` para que ya no digan "auto clásico"/"auto de lujo".

`src/data/vehiculos.ts` quedó con **7 vehículos**: `ford-a-1930-azul`, `mercury-1947-negro`, `kaiser-carabela-1959` (antiguos), `mercedes-benz-clase-e-2013`, `mercedes-benz-e400-2020`, `audi-a4-azul` (modernos), `limousine-chrysler-pt` (limousinas). `src/data/categorias.ts` y `CategoriaId` (`src/types/index.ts`) quedaron con esas 3 categorías. Todos los datos de las categorías/modelos descartados en el camino siguen recuperables desde el commit `d28ee5a` (tip de `main`, inventario completo de 206) si el cliente pide reincorporar algo más adelante.
...
**Recategorización aceptada, no verificada con el cliente sobre el fondo:** el Kaiser Carabela 1959 (1959, mitad de siglo) quedó dentro de "Antiguos" por decisión práctica del cliente para no tener una 4ª categoría, ampliando un poco la definición original de esa categoría ("primeras décadas del automóvil"). No es un error, pero vale mencionarlo si el cliente reordena categorías más adelante.
